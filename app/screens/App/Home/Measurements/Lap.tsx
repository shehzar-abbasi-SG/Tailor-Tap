...
const MeasurementScreenLap =({navigation}:IMeasurementNavigationProps)=> {
  ...
  return (
    <Layout>
      ...
      <Button className="self-center rounded-[3px] w-[75px] h-[34px]" buttonTextStyles="text-[13px] leading-[15px] font-bold font-[InterBold] uppercase p-0" onPress={()=>{}} title="Lap"/>
       <Image
          source={require('@/assets/images/lap_measurement.png')}
          alt="length measurement"
          className="w-full h-auto mt-5 "
          resizeMode="contain" 
        />
        <Formik
          initialValues={{ lap: '' } as PartialClientDetailFormData}
            ...
            {({ handleChange, handleBlur, handleSubmit,values, errors, touched }) => (
              ...
              <View className=''>
                <Input
                    variant="outline"
                    size="md"
                    isInvalid={touched.lap && Boolean(errors.lap)}
                  ...
                  {touched.lap && errors.lap && (
                    <Text className="font-[PoppinsRegular] text-sm mt-1 ml-1 text-red-500">{errors.lap}</Text>
                  )}
              </View>
              <View className='flex gap-y-[16px]'>
                ...
                <Button className="mt-0 bg-transparent h-[50px] rounded-[7px] border-[#38D55B] border-[2px]" buttonTextStyles="text-[22px] leading-[33px] uppercase text-[#38D55B] p-0" onPress={()=>navigation.navigate('Pant')} title="Skip"/>
              </View>
            </>
          )}
        </Formik>
...
export default MeasurementScreenLap