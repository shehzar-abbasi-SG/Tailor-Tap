          ...
          onSubmit={(values) => {
            ...
            {({ handleChange, handleBlur, handleSubmit, setFieldValue,values, errors, touched }) => (
              ...
              <View className=''>
                <Input
                  ...
                  {touched.length && errors.length && (
                    <Text className="font-[PoppinsRegular] text-sm mt-1 ml-1 text-red-500">{errors.length}</Text>
                  )}
              </View>
              <View className='flex gap-y-[16px]'>
                ...
                <Button className="mt-0 bg-transparent h-[50px] rounded-[7px] border-[#38D55B] border-[2px]" buttonTextStyles="text-[22px] leading-[33px] uppercase text-[#38D55B] p-0" onPress={()=>
                  navigation.navigate('Shoulder')
                } title="Skip"/>
              </View>
            </>
